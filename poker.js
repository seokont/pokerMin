function poker() {
    ...
    U.mouse = true;
    U.newAccounts = false;
    U.notecolor = ["", "#97FF30", "#669966", "#80FFE1", "#309BFF", "#FFBBFF", "#8E7AFF", "#FFAA30", "#FF3E30", "#FFDB30", "#B5946E"];
    U.notelabel = ["", "", "", "", "", "", "", "", "", "", ""];
    U.params = {};
    ...
    U.permissions = [];
    U.playerAction = [];
    U.postFlopButtons = ["Min", "20%", "25%", "30%", "33%", "35%", "40%", "45%", "50%", "55%", "60%", "65%", "67%", "70%", "75%", "80%", "Pot", "Max"];
    U.preFlopButtons = ["Min", "2½bb", "3bb", "3½bb", "4bb", "4½bb", "5bb", "5½bb", "6bb", "Pot", "Max"];
    U.proc = null;
    U.profileURL = "";
    U.pset = {};
    U.quit = false;
    ...
    function v() {
        ...
        s("MSG Version " + U.clientVersion + " : " + U.params.sitePlatform + " : " + U.params.siteID);
    ...
    function o() {
        ...
        U.fullScreenCancel = g.exitFullscreen || g.mozCancelFullScreen || g.webkitExitFullscreen || g.msExitFullscreen;
        U.fullScreenRequest = bg.requestFullscreen || bg.mozRequestFullScreen || bg.webkitRequestFullScreen || bg.msRequestFullscreen
    }

    function a4() {
        var g = window.document;
        return g.fullscreenElement || g.mozFullScreenElement || g.webkitFullscreenElement || g.msFullscreenElement
    }
    ...
    function aC(bA, bx, bv, bu, bt, br) {
        var bo, bs, bE, bD, bC, bB, by, bw, bz, bq, bp, bn, bm, bl, bk, bj, bi, bh, bg, g, bF;
        bo = "23456789TJQKA";
        bs = "cdhs";
        bz = "[";
        if (bA > 0 && bA < 53) {
    ...
    function a7(bi, bh, bk) {
        ...
        if (U.loggedIn && !U.mobile) {
            bg = " - " + U.lang.TableCaptionLoggedIn.split("%1%").join(U.loginData.player)
        }
        if (bh == "R") {
            for (g = 0; g < U.data.Ring.rows.length; g++) {
                if (U.data.Ring.rows[g].id == bi) {
                    return bi + (U.mobile ? "" : " - " + U.data.Ring.rows[g].game + " (" + U.data.Ring.rows[g].buyin + ")") + bg
                }
            }
            return "?"
        } else {
            ...
            for (g = 0; g < bj.length; g++) {
                if (bj[g].id == bm) {
                    return bi + (U.mobile ? "" : " - " + bj[g].game + " (" + bj[g].buyin + ")") + bg
                }
            }
            return "?"
        }
    ...
    function W(g) {
        if (g == 0) {
            ...
            return "<div class='grid_color' style='background-color: " + U.notecolor[g] + ";'>"
        }
    }
    ...
    function aN() {
        if (typeof AudioContext !== "undefined") {
            ...
            U.waGain = (typeof U.waContext.createGain == "undefined") ? U.waContext.createGainNode() : U.waContext.createGain();
            U.mp3 = true;
    ...
    function m() {
        ...
        bg = ["Min", "2½bb", "3bb", "3½bb", "4bb", "4½bb", "5bb", "5½bb", "6bb", "Pot", "Max"];
        bh = ["Min", "20%", "25%", "30%", "33%", "35%", "40%", "45%", "50%", "55%", "60%", "65%", "67%", "70%", "75%", "80%", "Pot", "Max"];
        bk = [2, 4, 6, 9];
        g = [4, 8, 12, 16];
    ...
    function aS(g) {
        ...
        U.ws.close();
        if (a4()) {
            bg = new A($("#FullScreenExitBtn"), U.lang.LobbyButtonExitFS, 40, function() {
                at(false);
                bg.show(false)
            });
            bg.show()
    ...
    function aR(g, bk) {
        ...
        bm = ["", "A", "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"];
        bl = (g & 983040) >>> 16;
        bj = (g & 61440) >>> 12;
        bi = (g & 3840) >>> 8;
        bh = (g & 240) >>> 4;
    ...
    function a9() {
        ...
        if (screen.width < screen.height) {
            $("#Connecting").text(params.connectRotate).css({
    ...
    function S() {
        ...
        aG();
        $("#Connecting").text(params.connectMsg).css({
            color: params.connectTextColor,
            "font-size": "1.75em"
        }).show();
        L()
    ...
    function P(bj, g, bh) {
        ...
        bg = window.open("", "", "scrollbars=yes, resizable=yes, width=640, height=480");
        bi = bg.document;
        bi.open();
        ...
        bi.writeln("<meta http-equiv='Content-Type' content='text/html; charset=utf-8'>");
        bi.writeln("</head>");
        bi.writeln("<body>");
        if (bh) {
            bi.writeln("<pre>")
    ...
    function p(bh, g) {
        ...
        if (U.waContext) {
            ...
            bi.onload = function() {
                U.waContext.decodeAudioData(bi.response, function(bk) {
                    bg.buffer = bk;
                    bg.loaded = true
                }, function() {
            ...
            bg.player.addEventListener("loadedmetadata", function() {
                bg.loaded = true
            }, false)
        }
    }
    ...
    function E(bi, bh, bg) {
        ...
        g.$box = $("<div>").addClass("checkbox").css("background-color", U.color.Window).appendTo(bi);
    ...
    function ay(bq, bj, bo, g, bh) {
        ...
        bg.$gridheader = $("<div>").addClass("grid_header").css({
            color: U.color.ButtonText,
            "background-color": U.color.Button,
            "border-color": U.color.ButtonBorder
        }).appendTo(bq);
        ...
        bg.selrow = -1;
        bg.sellast = -1;
        bg.vrows = Math.floor((bg.$grid.innerHeight() - bg.$gridheader.outerHeight()) / bg.data.rowHeight);
        bg.$gridcol = [];
        ...
        for (bk = 0; bk < bg.cols; bk++) {
            ...
            bm = $("<div>").width(bg.cw[bk]).html(bg.ch[bk]).css("border-right-color", U.color.ButtonBorder);
            if (bk == bg.data.sortCol && bg.data.sortable) {
                bm.append(bg.data.sortAscend ? " " + bg.sortA : " " + bg.sortD)
            }
            bg.$gridheader.append(bm);
            bg.$gridcol[bk] = $("<div>").width(bg.cw[bk]).css("border-right-color", U.color.ButtonBorder);
    ...
    ay.prototype.guiChange = function(bh) {
        ...
        if (U.params.gradients) {
            ...
            bg.scrollbar.$up.css("background-image", "url('Image?Name=Grad" + bp + "')");
            bg.scrollbar.$down.css("background-image", "url('Image?Name=Grad" + bp + "')");
            bg.scrollbar.$thumb.css("background-image", "url('Image?Name=Grad" + bp + "')")
        }
        bo = bh ? 32 : 16;
        bg.setRowHeight(bo)
    };
    ...
    ay.prototype.resize = function() {
        ...
        bk = bj.vrows;
        bj.vrows = Math.floor((bj.$grid.innerHeight() - bj.$gridheader.outerHeight()) / bj.data.rowHeight);
        bm = bj.vrows - bk;
        if (bm > 0) {
            for (bi = 0; bi < bj.cols; bi++) {
    ...
    function x(bi, bh) {
        ...
        g.$memotext = $(g.fixed ? "<pre>" : "<div>").addClass("memo noselect").css("color", U.color.ListText).appendTo(bi);
    ...
    x.prototype.guiChange = function(bh) {
        ...
        if (U.params.gradients) {
            bk = bh ? "32H" : "16H";
            bg.scrollbar.$up.css("background-image", "url('Image?Name=Grad" + bk + "')");
            bg.scrollbar.$down.css("background-image", "url('Image?Name=Grad" + bk + "')");
            bg.scrollbar.$thumb.css("background-image", "url('Image?Name=Grad" + bk + "')")
        }
    ...
    bb.prototype.add = function(bi, bk) {
        ...
        bh.$plate = $("<div>").addClass("minwin").css({
            left: g,
            color: U.color.WindowText,
            "background-color": U.color.Window,
            "border-color": U.color.Window
        }).text(bh.title).appendTo(bj.$box);
        bh.$hint = $("<div>").text(bh.title).appendTo(U.$webClient).addClass("tooltip").hide();
        bh.$plate.hover(function(bl) {
            bh.$hint.css({
    ...
    function l(g, bg) {
        ...
        bh.po = g;
        bh.$container = bg;
        bh.$bar = $("<div>").addClass("scrollbar").css({
            "background-color": U.color.Button,
            "border-color": U.color.ButtonBorder
        }).appendTo(bg);
        bh.$up = $("<div>").addClass("scrollbar_up").html(U.arrowU).appendTo(bh.$bar);
        bh.$thumb = $("<div>").addClass("scrollbar_thumb").appendTo(bh.$bar);
        bh.$down = $("<div>").addClass("scrollbar_down").html(U.arrowD).appendTo(bh.$bar);
        bh.$up.add(bh.$down).add(bh.$thumb).css({
            color: U.color.ButtonText,
            "border-color": U.color.ButtonBorder
        });
        if (U.params.gradients) {
            bh.$up.add(bh.$down).add(bh.$thumb).css("background-image", "url('Image?Name=Grad16H')")
        }
        bh.scale = 1;
        bh.ypos = 0;
        bh.ydown = 0;
    ...
    function w(bi, g, bm, bl) {
        ...
        bg.table = bi;
        bg.seat = bl;
        bg.$container = $(".seatplate").clone().removeClass("seatplate").appendTo(bi.$content).css({
            top: bm,
            left: g
        });
        bg.$cardbox = $("<div>").appendTo(bi.$content).css({
            top: bm - 38,
            left: g,
            width: 130,
            height: 38
    ...
    w.prototype.adjustSide = function() {
        ...
        g.$graphic.css("background", "url('Image?Name=Seat&Crc=" + U.crc.image + "') " + bh + "px 0px/auto 38px no-repeat");
    ...
    w.prototype.avatarSet = function(g) {
        ...
        this.$avatar.css("background", "url('Image?Name=Avatars&Crc=" + U.crc.image + "') " + bg)
    };
    w.prototype.avatarSetCustom = function(g, bg) {
        this.$avatar.css("background", "url('Avatar?Player=" + encodeURIComponent(g) + "&Crc=" + bg + "') 0px 0px/32px 32px no-repeat")
    };
    w.prototype.chatBlockIcon = function(g) {
        if (g) {
            this.$chatblock.css("background", "url('Image?Name=Block&Crc=" + U.crc.image + "') 0px 0px/10px 12px no-repeat")
        } else {
    ...
    w.prototype.chopIcon = function(g) {
        if (g) {
            this.$chop.css("background", "url('Image?Name=Chop&Crc=" + U.crc.image + "') 0px 0px/20px 10px no-repeat").show()
        } else {
            this.$chop.hide()
        }
    };
    w.prototype.clear = function() {
        ...
        this.setName("");
        this.setInfo("");
        this.$graphic.css("background", "url('Image?Name=SeatEmpty&Crc=" + U.crc.image + "') 0px 0px/auto 38px no-repeat");
    ...
    w.prototype.setName = function(bg) {
        ...
        g.$name.text(bg);
        if (bg == "") {
            g.$graphic.css("background", "url('Image?Name=SeatEmpty&Crc=" + U.crc.image + "') 0px 0px/auto 38px no-repeat");
    ...
    w.prototype.twiceIcon = function(g) {
        if (g) {
            this.$twice.css("background", "url('Image?Name=RunTwice&Crc=" + U.crc.image + "') 0px 0px/15px 10px no-repeat").show()
        } else {
            this.$twice.hide()
        }
    };
    ...
    function ao(bk, g, bh) {
        ...
        bj = this;
        bj.$container = bk;
        bj.$bar = $("<div>").addClass("slider_bar").css({
            "background-color": U.color.Button,
            "border-color": U.color.ButtonBorder
        }).appendTo(bk);
        bj.$thumb = $("<div>").addClass("slider_thumb").css({
            "background-color": U.color.Button,
            "border-color": U.color.ButtonBorder
        }).appendTo(bk);
        bj.onChange = bh;
    ...
    aH.prototype.Balance = function(g) {
        U.lobby.balanceShow(U.loginData.player, B(g.Available), B(g.Available2), B(g.InPlay), B(g.InPlay2), B(g.Total), B(g.Total2))
    };
    ...
    aH.prototype.Chat = function(bh) {
        ...
        if (bg == "") {
            ...
            br = "<font color='" + bq.color1 + "'>" + bk + bp + bg + ":  </font><font color='" + bq.color2 + "'>" + g + "</font>"
        }
        br = "<span>" + br + "</span>";
        if (bi) {
    ...
    aH.prototype.Language = function(bh) {
        ...
        U.playerAction = ["", U.lang.PlayerActionAddOn, U.lang.PlayerActionAllIn, U.lang.PlayerActionAnte, U.lang.PlayerActionBB, U.lang.PlayerActionBet, U.lang.PlayerActionBlind, U.lang.PlayerActionBringIn, U.lang.PlayerActionCall, U.lang.PlayerActionCheck, U.lang.PlayerActionFold, U.lang.PlayerActionRaise, U.lang.PlayerActionRebuy, U.lang.PlayerActionReserved, U.lang.PlayerActionSB, U.lang.PlayerActionSBBB, U.lang.PlayerActionSit, U.lang.PlayerActionStraddle, U.lang.PlayerActionWait]
    };
    ...
    aH.prototype.LobbyChat = function(bh) {
        ...
        if (bg == "") {
            ...
            bp = "<font color='" + bo.color1 + "'>" + bl + bn + bg + ":  </font><font color='" + bo.color2 + "'>" + g + "</font>"
        }
        bp = "<span>" + bp + "</span>";
        if (bi) {
    ...
    aH.prototype.OpenTable = function(bj) {
        var bu, bn, g, by, bx, bh, bk, bl, bq, bp, bv, bi, br, bo, bt, bw, bg, bs, bm;
        ...
        if (bu) {
            ...
            bu = new an(by, bn, g, bx, limit, bi, bh, bk, bl, bq, bp, bv, bo, bt, bw, bg, bs, U.winOfsX, U.winOfsY);
            y();
            U.tables.push(bu);
            bu.bringToFront();
            a2(bu.dialog);
    ...
    aH.prototype.PlayerInfo = function(bg) {
        ...
        if (bl == "R") {
            ...
            if (bj == "") {
                ...
                bo = U.lang.LobbyCaptionRunning.split("%1%").join(g).split("%2%").join(bj)
            }
            if (bi) {
    ...
    aH.prototype.PlayerSettings = function(bl) {
        ...
        if ("RingPlayerSortColumn" in bj) {
            ...
            if (bi != U.data.RingPlayer.sortCol || g != U.data.RingPlayer.sortAscend) {
        ...
        if ("SitnGoPlayerSortColumn" in bj) {
            ...
            if (bi != U.data.SitnGoPlayer.sortCol || g != U.data.SitnGoPlayer.sortAscend) {
        ...
        if ("TourneyPlayerSortColumn" in bj) {
            ...
            if (bi != U.data.TourneyPlayer.sortCol || g != U.data.TourneyPlayer.sortAscend) {
        ...
        bh = ["Activate", "Holdem", "Omaha", "OmahaHiLo", "Omaha5", "Omaha5HiLo", "Razz", "Stud", "StudHiLo", "Mixed", "NL", "PL", "CL", "Fixed", "StakesMin", "StakesMax", "BuyinMin", "BuyinMax", "SeatsMin", "SeatsMax", "PlayersMin", "Primary", "Secondary", "HideFull", "HidePrivate"];
        bg = ["Activate", "Holdem", "Omaha", "OmahaHiLo", "Omaha5", "Omaha5HiLo", "Razz", "Stud", "StudHiLo", "Mixed", "NL", "PL", "Fixed", "Freezeout", "Rebuy", "Reentry", "Shootout", "BuyinMin", "BuyinMax", "SizeMin", "SizeMax", "Time", "Other", "Primary", "Secondary", "HidePrivate", "HideRunning"];
        if (bk("filterRing", bh)) {
    ...
    aH.prototype.RingGameLobby = function(bh) {
        ...
        for (bm = 0; bm < bo; bm++) {
            ...
            bp = {};
            bp.id = bg;
            bp.bold = (U.sitting.indexOf("R" + bg) > -1) || (U.waiting.indexOf("R" + bg) > -1);
            ...
            bp.stakesSort = g + ("0000000000" + au(bp.stakesLo).toFixed(2)).substr(-14);
            ...
            bp.buyinSort = g + ("0000000000" + au(bp.buyinMin).toFixed(2)).substr(-14);
    ...
    aH.prototype.Session = function(bk) {
        ...
        U.$webClient.add("#OpenBackground").css("background-color", U.color.Background);
        if (bk.BackgroundGraphic == "Default") {
            ...
            bi = $("#BGLogo").clone().attr("id", "BGLogo2").appendTo("#OpenBackground");
            $("a", bi).removeAttr("href").removeAttr("target")
        } else {
            if (bk.BackgroundGraphic == "Yes") {
                if (bk.BackgroundTile == "Yes") {
                    $("#BGTile").css("background-image", "url('Image?Name=Logo&Crc=" + U.crc.image + "')");
                    bj = aq(bk.BackgroundLink);
                    if (bj != "") {
                        $("#BGTile").attr("href", bj)
                    }
                    $("#BGTile").show();
                    bi = $("#BGTile").clone().attr("id", "BGTile2").appendTo("#OpenBackground");
    ...
    aH.prototype.Table = function(bm) {
        ...
        for (bi = 1; bi <= 10; bi++) {
            ...
            bl.playerTime[bi] = U.lang.GameSeconds.split("%1%").join(aq(bm.Time[bi - 1]));
            ...
            bl.holeCard[1][bi] = (g && !bl.isFaceDown) ? bl.cardNum[1] : au(bm.Card1[bi - 1]);
            bl.holeCard[2][bi] = (g && !bl.isFaceDown) ? bl.cardNum[2] : au(bm.Card2[bi - 1]);
            bl.holeCard[3][bi] = (g && !bl.isFaceDown) ? bl.cardNum[3] : au(bm.Card3[bi - 1]);
            bl.holeCard[4][bi] = (g && !bl.isFaceDown) ? bl.cardNum[4] : au(bm.Card4[bi - 1]);
            bl.holeCard[5][bi] = (g && !bl.isFaceDown) ? bl.cardNum[5] : au(bm.Card5[bi - 1]);
            bl.holeCard[6][bi] = (g && !bl.isFaceDown) ? bl.cardNum[6] : au(bm.Card6[bi - 1]);
            bl.holeCard[7][bi] = (g && !bl.isFaceDown) ? bl.cardNum[7] : au(bm.Card7[bi - 1])
        }
        for (bi = 1; bi <= 9; bi++) {
    ...
    aH.prototype.TournamentLobby = function(bp) {
        var bB, bz, bw, bt, bq, bh, bg, bC, bE, bo, bs, bx, by, bv, bl, bj, bu, bn, bD, bk, bA, g, bm, br, bi;
        ...
        for (bB = 0; bB < bn; bB++) {
            ...
            g = {};
            g.id = bt;
            g.bold = (U.sitting.indexOf("T" + bt) > -1) || (U.waiting.indexOf("T" + bt) > -1);
            ...
            if (g.starts == "Time") {
                g.starts = U.lang.StatusStartsTime.split("%1%").join(a5(bx, false, br, false));
                g.startsSort = U.lang.StatusStartsTime.split("%1%").join(a5(bx, false, false, true))
            }
    ...
    aH.prototype.Transactions = function(bg) {
        ...
        U.lobby.transactions.controls.$count.text(U.lang.TransactionsRecords + " " + U.data.Trans.rows.length);
    ...
    function bd(bk) {
        ...
        bj.$closeBtn = $(".closebtn", bj.$dialog).on("touchstart mousedown", function(bn) {
            bj.close();
            return false
        });
        ...
        bj.sitnGoTabSetup();
        bj.mobileInfoSetup();
        g = [U.lang.LobbyCaptionLogins, U.lang.LobbyCaptionRingGames, U.lang.LobbyCaptionTournaments, U.lang.LobbyCaptionSitnGos, U.lang.LobbyCaptionOpen];
        bi = [true, true, true, U.params.sitAndGoTab, false];
        bj.lobbyTabs = new ar($("#LobbyTabs", bj.$dialog), g, bi, function(bn, bo) {
    ...
    bd.prototype.aboutShow = function() {
        ...
        $("#about_copyright").html("Copyright &copy; " + U.copyright + " Kent Briggs");
    ...
    bd.prototype.accountCustomShow = function() {
        ...
        g = g + "Player=" + encodeURIComponent(U.loginData.player) + "&Key=" + encodeURIComponent(U.loginData.playerKey);
        window.open(g, "_blank")
    };
    bd.prototype.accountInfoAvatar = function(bi) {
        ...
        bg = this;
        bj = bg.accountInfo;
        bj.data.aiAvatar = Math.round(bi * (U.maxAvatar - U.minAvatar)) + U.minAvatar;
        $("#ai_avatar_label").text(U.lang.AccountAvatar + " " + bj.data.aiAvatar);
        g = bj.data.aiAvatar;
        if (g == 0) {
            $("#ai_avatar_image").css("background", "url('Avatar?Player=" + encodeURIComponent(U.loginData.player) + "&Crc=" + U.loginData.avatarCrc + "') 0px 0px/64px 64px no-repeat")
        } else {
            bh = ((g - 1) * -64) + "px 0px/auto 64px no-repeat";
            $("#ai_avatar_image").css("background", "url('Image?Name=Avatars&Crc=" + U.crc.image + "') " + bh)
        }
    };
    ...
    bd.prototype.accountInfoCreate = function() {
        ...
        $("#ai_player_desc").text(U.lang.AccountPlayerDesc).css("color", U.color.ListDisabled);
        ...
        $("#ai_pw_desc").text(U.lang.AccountPWDesc).css("color", U.color.ListDisabled);
        $("#ai_email_label").text(U.lang.AccountEmail);
        bl.controls.aiEmail = new ax($("#ai_email_input"), {
            border: true
        });
        $("#ai_email_desc").text(U.lang.AccountEmailDesc).css("color", U.color.ListDisabled);
        $("#ai_avatar_label").text(U.lang.AccountAvatar);
        new A($("#ai_avatar_prev"), U.arrowL, 20, function() {
            bh.accountInfoAvatarPrev()
        });
        bl.controls.avatarSlider = new ao($("#ai_avatar_slider"), 1 / (U.maxAvatar - U.minAvatar + 1), function(bm) {
            bh.accountInfoAvatar(bm)
        });
        new A($("#ai_avatar_next"), U.arrowR, 20, function() {
            bh.accountInfoAvatarNext()
        });
        $("#ai_avatar_desc").text(U.lang.AccountAvatarDesc).css("color", U.color.ListDisabled);
        bi = 330;
        $("#ai_real").css("top", bi);
        ...
        $("#ai_real_desc").text(U.lang.AccountRealDesc).css("color", U.color.ListDisabled);
        if (U.realnameEnable) {
            $("#ai_real").show();
            bi = bi + 80
        }
        ...
        $("#ai_loc_desc").text(U.lang.AccountLocationDesc).css("color", U.color.ListDisabled);
        if (U.locationEnable) {
            $("#ai_loc").show();
            bi = bi + 80
        }
        ...
        bl.controls.aiOther = new C($("#ai_gender_other"), U.lang.AccountUnspecified);
    ...
    bd.prototype.accountSuspendOk = function() {
        ...
        g = au(bh.controls.mins.getText()) + au(bh.controls.hours.getText()) * 60 + au(bh.controls.days.getText()) * 1440;
        bh.close();
        j({
            Response: "Suspend",
            Minutes: g
    ...
    bd.prototype.arrangeTile = function() {
        var bv, bw, g, bs, bx, br, bi, by, bg, bo, bt, bl, bj, bh, bm, bq, bp, bu, bk, bn;
        bv = this;
    ...
    bd.prototype.cardDeckCreate = function() {
        ...
        for (bh = 1; bh < 5; bh++) {
            ...
            $("#cd_" + bh + "a", g).css("background", "url('" + bg + "') -1656px 0px/auto 64px");
            $("#cd_" + bh + "b", g).css("background", "url('" + bg + "') -1886px 0px/auto 64px");
            $("#cd_" + bh + "c", g).css("background", "url('" + bg + "') -2116px 0px/auto 64px");
            $("#cd_" + bh + "d", g).css("background", "url('" + bg + "') -2346px 0px/auto 64px");
            $("#cd_" + bh + "e", g).css("background", "url('" + bg + "') -2392px 0px/auto 64px")
        }
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
            for (bh = 1; bh < 5; bh++) {
                if (bj.controls.rc[bh].isChecked()) {
                    bi.tableSettings.data.deckNum = bh;
                    bi.tableSettings.controls.deckBtn.setCaption(U.lang.TableSettingsCardDeck + ": #" + bh);
                    break
                }
            }
            bj.close()
    ...
    bd.prototype.contactShow = function() {
        ...
        if (g == "") {
            ...
            $("#ca_email").attr("href", "mailto:" + g + "?subject=" + encodeURIComponent(U.siteName)).text(g)
        }
        if (U.siteWeb == "") {
    ...
    bd.prototype.displaySettingsCreate = function() {
        ...
        bh.controls.echoAsterisk = new C($("#EchoAsterisk"), U.lang.DisplayAsterisk);
        bh.controls.echoNothing = new C($("#EchoNothing"), U.lang.DisplayNothing);
        $("#ChatTimestamps").text(U.lang.DisplayChatTime);
        bh.controls.lobbyChatTime = new E($("#LobbyChatTime"), U.lang.DisplayLobbyChatTime);
        bh.controls.tableChatTime = new E($("#TableChatTime"), U.lang.DisplayTableChatTime);
    ...
    bd.prototype.guiChange = function() {
        ...
        if (U.mobile) {
            ...
            bg.$loginSelected.add(bg.$ringSelected).add(bg.$tourneySelected).add(bg.$sitnGoSelected).css("font-size", "1.2em")
        } else {
            bg.$dialog.css({
                left: U.winOfsX,
                top: U.winOfsY,
            ...
            bg.$loginSelected.add(bg.$ringSelected).add(bg.$tourneySelected).add(bg.$sitnGoSelected).css("font-size", "1.0em")
        }
    ...
    bd.prototype.languageCreate = function() {
        ...
        $("#LanguageNote").text(U.lang.LanguageNote).css("color", U.color.ListDisabled);
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
            bg.languageOk()
        });
        new A($(".cancel", g), U.lang.DialogCancel, 25, function() {
    ...
    bd.prototype.lobbyChatUpdate = function() {
        ...
        for (bh = 0; bh < U.lobbyChatQueue.length; bh++) {
            ...
            bm = bm + "<span>";
            if (bn.player != "") {
                bm = bm + "<font color='" + bn.color1 + "'>" + bj + bn.title + bn.player + ":  </font><font color='" + bn.color2 + "'>" + bn.text + "</font>"
            }
            bm = bm + "</span><br>"
        }
        bi.setText(bm);
    ...
    bd.prototype.lobbyTabsChange = function(g, bj) {
        ...
        if (g == 1) {
            ...
            if (bg >= 0 && bg < U.data.Ring.rows.length && U.data.Ring.rows[bg].startCode) {
                bi = true
            }
        } else {
            if (g == 2) {
                bg = bh.tourneyGrid.selrow;
                if (bg >= 0 && bg < U.data.Tourney.rows.length && U.data.Tourney.rows[bg].startCode) {
                    bi = true
                }
            } else {
                if (g == 3) {
                    bg = bh.sitnGoGrid.selrow;
                    if (bg >= 0 && bg < U.data.SitnGo.rows.length && U.data.SitnGo.rows[bg].startCode) {
                        bi = true
                    }
                }
            }
    ...
    bd.prototype.loginCreate = function() {
        ...
        bh.controls.loginCreateBtn = new A($("#li_createbtn"), U.lang.LoginAccount, 20, function() {
            bh.close();
            bg.accountCreateShow()
        });
        bh.controls.loginResetBtn = new A($("#li_resetbtn"), U.lang.LoginResetPW, 20, function() {
            bg.resetPassword()
        });
        bh.controls.loginResendBtn = new A($("#li_resendbtn"), U.lang.LoginResendVC, 20, function() {
            bg.resendValCode()
        });
        bh.controls.$loginValCodeLabel = $("#li_valcodelabel").text(U.lang.LoginValCode);
    ...
    bd.prototype.loginShow = function() {
        ...
        bi.controls.$loginValCodeLabel.toggle(U.passwordRecovery || U.validateEmails);
    ...
    bd.prototype.loginTabSetup = function() {
        ...
        U.data.Login.headers = [U.lang.LobbyColumnLoginsPlayer, U.lang.LobbyColumnLoginsColor, U.lang.LobbyColumnLoginsNote, U.lang.LobbyColumnLoginsBlock, U.lang.LobbyColumnLoginsTime];
        U.data.Login.fields = ["player", "real", "location", "color", "colorSort", "note", "block", "login", "loginSort"];
        U.data.Login.fieldsShow = ["player", "color", "note", "block", "login"];
        U.data.Login.fieldsSort = ["player", "colorSort", "note", "block", "loginSort"];
        ...
        g.loginGrid = new ay($("#LoginGrid", g.$dialog), U.data.Login, function(bg) {
            g.loginSelect(bg)
        }, function() {
        ...
        g.lobbyChatBtn = new A($("#LobbyChatBtn", g.$dialog), U.lang.LobbyButtonChatExt, 25, function() {
            g.popoutChatShow()
        });
        ...
        g.lobbyChatSendBtn = new A($("#LobbyChatSendBtn", g.$dialog), "&#8595;", 25, function() {
        ...
        g.searchBtn = new A($("#SearchBtn", g.$dialog), U.lang.LobbyButtonSearch, 40, function() {
            g.playerSearchShow()
        });
        g.notesBtn = new A($("#NotesBtn", g.$dialog), U.lang.LobbyButtonNotes, 40, function() {
            g.noteListShow()
        });
        g.balanceBtn = new A($("#BalanceBtn", g.$dialog), U.lang.LobbyButtonBalance, 40, function() {
            j({
                Response: "Balance"
            })
        });
        g.logInOutBtn = new A($("#LogInOutBtn", g.$dialog), U.lang.LobbyButtonLogin, 40, function() {
            if (U.loggedIn == false) {
                g.loginShow()
            } else {
                g.close()
    ...
    bd.prototype.menuInit = function() {
        ...
        bg.lobbyLogins = new ad($("#LobbyLogins", bg.$dialog), U.lang.LobbyCaptionLogins, function() {
            bg.lobbyTabs.setTab(0)
        }, true);
        bg.lobbyRingGames = new ad($("#LobbyRingGames", bg.$dialog), U.lang.LobbyCaptionRingGames, function() {
            bg.lobbyTabs.setTab(1)
        }, true);
        bg.lobbyTournaments = new ad($("#LobbyTournaments", bg.$dialog), U.lang.LobbyCaptionTournaments, function() {
            bg.lobbyTabs.setTab(2)
        }, true);
        bg.lobbySitnGos = new ad($("#LobbySitnGos", bg.$dialog), U.lang.LobbyCaptionSitnGos, function() {
            bg.lobbyTabs.setTab(3)
        }, true);
        bg.lobbySitnGos.show(U.params.sitAndGoTab);
        bg.lobbyOpenTables = new ad($("#LobbyOpenTables", bg.$dialog), U.lang.LobbyCaptionOpen, function() {
            bg.lobbyTabs.setTab(4)
        }, true);
        bg.lobbyOpenTables.show(U.mobile);
        bg.lobbyDefault = new ad($("#LobbyDefaultSize", bg.$dialog), U.lang.LobbyMenuWindowSize, function() {
            bg.defaultSize()
        }, true);
        bg.lobbyExit = new ad($("#LobbyExit", bg.$dialog), U.lang.LobbyMenuLobbyExit, function() {
            bg.close()
        }, true);
        $("#AccountMenu", bg.$dialog).text(U.lang.LobbyMenuAccount);
        bg.accountLogin = new ad($("#AccountLogin", bg.$dialog), U.lang.LobbyMenuAccountLogin, function() {
            bg.loginShow()
        }, true);
        bg.accountLogout = new ad($("#AccountLogout", bg.$dialog), U.lang.LobbyMenuAccountLogout, function() {
            bg.close()
        }, true);
        bg.accountCreate = new ad($("#AccountCreate", bg.$dialog), U.lang.LobbyMenuAccountCreate, function() {
            bg.accountCreateShow()
        }, true);
        bg.accountChange = new ad($("#AccountChange", bg.$dialog), U.lang.LobbyMenuAccountChange, function() {
            bg.accountChangeShow()
        }, true);
        bg.accountBalance = new ad($("#AccountBalance", bg.$dialog), U.lang.LobbyMenuAccountBalance, function() {
            j({
                Response: "Balance"
            })
        }, false);
        bg.accountTickets = new ad($("#AccountTickets", bg.$dialog), U.lang.LobbyMenuAccountTickets, function() {
            j({
                Response: "Tickets"
            })
        }, false);
        bg.accountPermissions = new ad($("#AccountPermissions", bg.$dialog), U.lang.LobbyMenuAccountPerm, function() {
            j({
                Response: "Permissions"
            })
        }, false);
        g = U.secondary ? U.lang.LobbyMenuAccountTransfer1 : U.lang.LobbyMenuAccountTransfer;
        bg.accountTransfer1 = new ad($("#AccountTransfer1", bg.$dialog), g, function() {
            bg.chipTransferShow(true)
        }, false);
        bg.accountTransfer2 = new ad($("#AccountTransfer2", bg.$dialog), U.lang.LobbyMenuAccountTransfer2, function() {
            bg.chipTransferShow(false)
        }, false);
        g = U.secondary ? U.lang.LobbyMenuAccountRequest1 : U.lang.LobbyMenuAccountRequest;
        bg.accountChipRequest1 = new ad($("#AccountChipRequest1", bg.$dialog), g, function() {
            j({
                Response: "BalanceReset",
                Primary: "Yes"
            })
        }, false);
        bg.accountChipRequest2 = new ad($("#AccountChipRequest2", bg.$dialog), U.lang.LobbyMenuAccountRequest2, function() {
            j({
                Response: "BalanceReset",
                Primary: "No"
            })
        }, false);
        bg.accountSelfSuspend = new ad($("#AccountSelfSuspend", bg.$dialog), U.lang.LobbyMenuAccountSuspend, function() {
            bg.accountSuspendShow()
        }, false);
        bg.accountCustom = new ad($("#AccountCustom", bg.$dialog), U.lang.LobbyMenuAccountCustom, function() {
            bg.accountCustomShow()
        }, false);
        $("#OptionsMenu", bg.$dialog).text(U.lang.LobbyMenuOptions);
        bg.optionsFullScreen = new ad($("#OptionsFullScreen", bg.$dialog), U.lang.LobbyMenuOptionsFullScreen, function() {
            bg.fullScreenToggle()
        }, true);
        bg.optionsLanguage = new ad($("#OptionsLanguage", bg.$dialog), U.lang.LobbyMenuOptionsLanguage, function() {
            bg.languageShow()
        }, true);
        bg.optionsArrange = new ad($("#OptionsArrange", bg.$dialog), U.lang.LobbyMenuOptionsArrange, function() {
            bg.arrangeWindowsShow()
        }, true);
        bg.optionsDisplay = new ad($("#OptionsDisplay", bg.$dialog), U.lang.LobbyMenuOptionsDisplay, function() {
            bg.displaySettingsShow()
        }, true);
        bg.optionsTable = new ad($("#OptionsTable", bg.$dialog), U.lang.LobbyMenuOptionsTable, function() {
            bg.tableSettingsShow()
        }, true);
        bg.optionsSound = new ad($("#OptionsSound", bg.$dialog), U.lang.LobbyMenuOptionsSound, function() {
            bg.soundShow()
        }, true);
        bg.optionsStart = new ad($("#OptionsStart", bg.$dialog), U.lang.LobbyMenuOptionsStart, function() {
            bg.startGameShow()
        }, false);
        bg.optionsSearch = new ad($("#OptionsSearch", bg.$dialog), U.lang.LobbyMenuOptionsSearch, function() {
            bg.playerSearchShow()
        }, true);
        bg.optionsNotes = new ad($("#OptionsNotes", bg.$dialog), U.lang.LobbyMenuOptionsNotes, function() {
            bg.noteListShow()
        }, true);
        $("#HelpMenu", bg.$dialog).text(U.lang.LobbyMenuHelp).on("touchstart mousedown", function(bh) {
            if (r(bh)) {
                return
            }
            U.doc.debug = true;
        ...
        bg.helpContact = new ad($("#HelpContact", bg.$dialog), U.lang.LobbyMenuHelpContact, function() {
            bg.contactShow()
        }, true);
        bg.helpNews = new ad($("#HelpNews", bg.$dialog), U.lang.LobbyMenuHelpNews, function() {
            j({
                Response: "News"
            })
        }, true);
        bg.helpFAQ = new ad($("#HelpFAQ", bg.$dialog), U.lang.LobbyMenuHelpFAQ, function() {
            j({
                Response: "FAQ"
            })
        }, true);
        bg.helpAbout = new ad($("#HelpAbout", bg.$dialog), U.lang.LobbyMenuHelpAbout + " Poker Mavens...", function() {
            bg.aboutShow()
        }, true);
        $(".menu", bg.$dialog).css({
            color: U.color.ButtonText,
    ...
    bd.prototype.mobileIconSetup = function() {
        ...
        bi.$mobileIconHint = $("<div>").appendTo(bi.$dialog).addClass("tooltip").hide();
        if (U.params.gradients) {
            $("#IconBar", bi.$dialog).css("background-image", "url('Image?Name=Grad30')")
        }
        ...
        bh = ["Lobby", "Chat", "Chips", "Refresh", "Rotate", "Replay", "Default", "Settings", "Leave"];
        bj = [U.lang.LobbyMenuLobby, U.lang.TableMenuChatInfo, U.lang.TableMenuAddChips, U.lang.TableMenuRefresh, U.lang.TableMenuRotate, U.lang.TableMenuReplay, U.lang.TableMenuWindowSize, U.lang.TableMenuSettings, U.lang.TableMenuLeave];
        for (bg = 0; bg < 9; bg++) {
            if (bg == 1 || bg == 6) {
                continue
            }
            g = $("#IconBar" + bh[bg], bi.$dialog);
            g.css("background", "url('Image?Name=TableIcons&Crc=" + U.crc.image + "') " + (-24 * bg - bk) + "px 0px/auto 24px no-repeat");
            g.css("opacity", 0.5);
            g.data("hint", bj[bg]);
            g.data("index", bg);
            g.hover(function(bl) {
    ...
    bd.prototype.mobileInfoSetup = function() {
        ...
        g = [U.lang.InfoChat, U.lang.InfoGeneral, U.lang.InfoStats, U.lang.InfoHistory];
        ...
        new A($("#mChatInfoBtn", bh.$dialog), U.lang.TableButtonChat, 25, function() {
        ...
        bh.historySlider = new ao($("#mHistorySlider", bh.$dialog), 1, function(bi) {
            U.tables[U.tableCurrent].infoDialog.controls.historySlider.setValue(bi, true)
        });
        ...
        bh.mControlsHideBtn = new A($("#mControlsHideBtn", bh.$dialog), U.lang.DialogHide, 30, function() {
            bh.mobileToggleControls(false)
        });
        bh.mInfoSaveBtn = new A($("#mInfoSaveBtn", bh.$dialog), U.lang.DialogSave, 30, function() {
            bh.mobileInfoSave()
        });
        bh.prevTableBtn = new A($("#PrevTableBtn", bh.$dialog), U.arrowL + " " + U.lang.TableButtonPrev, 30, function() {
            bh.prevTable()
        });
        bh.nextTableBtn = new A($("#NextTableBtn", bh.$dialog), U.lang.TableButtonNext + " " + U.arrowR, 30, function() {
            bh.nextTable()
        });
        bh.mControlsUnhideBtn = new A($("#mControlsUnhideBtn", bh.$dialog), "<", "16H", function() {
    ...
    bd.prototype.noteListCreate = function() {
        ...
        U.data.Notes.headers = [U.lang.NotePlayer, U.lang.NoteColor, U.lang.NoteNote, U.lang.NoteBlock];
        U.data.Notes.fields = ["player", "color", "note", "block", "colorNum", "noteText", "chatBool"];
        ...
        bh.controls.noteGrid = new ay($("#nl_grid", g), U.data.Notes, function(bi) {
    ...
    bd.prototype.postFlopButtonsCreate = function() {
        ...
        bi.controls.post = [];
        for (bg = 0; bg < 18; bg++) {
            bi.controls.post[bg] = new C($("#post_" + bg), U.postFlopButtons[bg], bh.postFlopButtonsSelect(bh, bg))
        }
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
    ...
    bd.prototype.preFlopButtonsCreate = function() {
        ...
        bi.controls.pre = [];
        for (bg = 0; bg < 11; bg++) {
            bi.controls.pre[bg] = new C($("#pre_" + bg), U.preFlopButtons[bg], bh.preFlopButtonsSelect(bh, bg))
        }
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
            bh.preFlopButtonsOk()
        });
    ...
    bd.prototype.ringFilterCreate = function() {
        ...
        bh.controls.rfOmahaXHiLo = new E($("#rf_omahaxhilo"), U.lang.FilterOmahaXHiLo);
        ...
        bh.controls.rfHidePrivate = new E($("#rf_hideprivate"), U.lang.FilterPrivate);
    ...
    bd.prototype.ringPlayersCreate = function() {
        ...
        bg.ringPrevBtn = new A($("#RingPrevBtn", g), U.lang.LobbyButtonRingPrev, 25, function() {
            bg.ringGamePrev()
        });
        bg.ringNextBtn = new A($("#RingNextBtn", g), U.lang.LobbyButtonRingNext, 25, function() {
            bg.ringGameNext()
        });
        bg.ringWait2Btn = new A($("#RingWait2Btn", g), U.lang.LobbyButtonRingJoin, 25, function() {
            bg.ringRegister()
        });
        new A($(".okbtn", g), U.lang.DialogOK, 25, function() {
            bh.close()
    ...
    bd.prototype.ringTabSetup = function() {
        ...
        U.data.Ring.headers = [U.lang.LobbyColumnRingID, U.lang.LobbyColumnRingGame, U.lang.LobbyColumnRingStakes, U.lang.LobbyColumnRingBuyIn, U.lang.LobbyColumnRingSeats, U.lang.LobbyColumnRingPlay, U.lang.LobbyColumnRingWait];
        U.data.Ring.fields = ["id", "game", "gameIndex", "stakes", "stakesLo", "stakesHi", "stakesSort", "buyin", "buyinMin", "buyinMax", "buyinSort", "seats", "playing", "waiting", "password"];
        U.data.Ring.fieldsShow = ["id", "game", "stakes", "buyin", "seats", "playing", "waiting"];
        U.data.Ring.fieldsSort = ["id", "game", "stakesSort", "buyinSort", "seats", "playing", "waiting"];
        ...
        g.ringGrid = new ay($("#RingGrid", g.$dialog), U.data.Ring, function(bh) {
            g.ringGameSelect(bh)
        }, function(bh) {
            g.ringTableOpen(bh)
        }, function() {
        ...
        g.ringFilterBtn = new A($("#RingFilterBtn", g.$dialog), bg, 40, function() {
            g.ringFilterShow()
        });
        g.ringObserveBtn = new A($("#RingObserveBtn", g.$dialog), U.lang.LobbyButtonRingObserve, 40, function() {
            g.ringTableSelectedOpen()
        });
        g.ringInfoBtn = new A($("#RingInfoBtn", g.$dialog), U.lang.LobbyButtonRingInfo, 40, function() {
            g.ringInfoRequest()
        });
        g.ringPlayersBtn = new A($("#RingPlayersBtn", g.$dialog), U.lang.LobbyButtonRingPlayers, 40, function() {
            g.ringPlayersShow()
        });
        g.ringWaitBtn = new A($("#RingWaitBtn", g.$dialog), U.lang.LobbyButtonRingJoin, 40, function() {
            g.ringRegister()
        });
        g.ringSelected = "";
        ...
        U.data.RingPlayer.headers = [U.lang.LobbyColumnRingPlayer, U.lang.LobbyColumnRingChips, U.lang.LobbyColumnRingNet];
        U.data.RingPlayer.fields = ["player", "chips", "chipsSort", "net", "netSort"];
        ...
        g.ringPlayerGrid = new ay($("#RingPlayerGrid", g.ringPlayers.$dialog), U.data.RingPlayer, null, null, function() {
        ...
        g.ringWaitGrid = new ay($("#RingWaitGrid", g.ringPlayers.$dialog), U.data.RingWait)
    };
    ...
    bd.prototype.sitnGoFilterData = function() {
        ...
        if (!U.pset.filterSitnGoActivate) {
            ...
            for (bh = 0; bh < U.data.Tourney.urows.length; bh++) {
                ...
                if (!U.pset.filterSitnGoFreezeout && !bn.rebuy && !bn.reentry && !bn.shootout) {
                    continue
                }
    ...
    bd.prototype.sitnGoPlayersCreate = function() {
        ...
        bg.sitnGoPrevBtn = new A($("#SitnGoPrevBtn", g), U.lang.LobbyButtonSnGPrev, 25, function() {
            bg.sitnGoPrev()
        });
        bg.sitnGoNextBtn = new A($("#SitnGoNextBtn", g), U.lang.LobbyButtonSnGNext, 25, function() {
            bg.sitnGoNext()
        });
        bg.sitnGoRegister2Btn = new A($("#SitnGoRegister2Btn", g), U.lang.LobbyButtonTrnyRegister, 25, function() {
            bg.sitnGoRegister()
        });
        bg.sitnGoStartNow = new E($("#SitnGoStartNow", g), U.lang.LobbyCaptionStartNow + " *", function() {
    ...
    bd.prototype.sitnGoRegister = function() {
        ...
        bh = (bk == U.lang.LobbyButtonTrnyRegister || bk == U.lang.LobbyButtonTrnyRegLate);
        if (bh) {
            j({
    ...
    bd.prototype.sitnGoTabSetup = function() {
        ...
        U.data.SitnGo.headers = [U.lang.LobbyColumnSnGID, U.lang.LobbyColumnTrnyGame, U.lang.LobbyColumnTrnyBuyIn, U.lang.LobbyColumnTrnyTS, U.lang.LobbyColumnTrnyReg, U.lang.LobbyColumnTrnyStarts];
        U.data.SitnGo.fields = ["id", "game", "gameIndex", "buyin", "buyinSort", "ts", "reg", "regSort", "tables", "starts", "startsSort", "startMin", "startTime", "password"];
        U.data.SitnGo.fieldsShow = ["id", "game", "buyin", "ts", "reg", "starts"];
        U.data.SitnGo.fieldsSort = ["id", "game", "buyinSort", "ts", "regSort", "startsSort"];
        ...
        g.sitnGoGrid = new ay($("#SitnGoGrid", g.$dialog), U.data.SitnGo, function(bh) {
            g.sitnGoSelect(bh)
        }, function(bh) {
        ...
        g.sitnGoFilterBtn = new A($("#SitnGoFilterBtn", g.$dialog), bg, 40, function() {
            g.tourneyFilterShow(true)
        });
        g.sitnGoObserveBtn = new A($("#SitnGoObserveBtn", g.$dialog), U.lang.LobbyButtonTrnyObserve, 40, function() {
            g.sitnGoTableSelectedOpen()
        });
        g.sitnGoInfoBtn = new A($("#SitnGoInfoBtn", g.$dialog), U.lang.LobbyButtonSnGInfo, 40, function() {
            g.sitnGoInfoRequest()
        });
        g.sitnGoPlayersBtn = new A($("#SitnGoPlayersBtn", g.$dialog), U.lang.LobbyButtonTrnyPlayers, 40, function() {
            g.sitnGoPlayersShow()
        });
        g.sitnGoRegisterBtn = new A($("#SitnGoRegisterBtn", g.$dialog), U.lang.LobbyButtonTrnyRegister, 40, function() {
            g.sitnGoRegister()
        });
        g.sitnGoSelected = "";
        ...
        U.data.SitnGoPlayer.headers = [U.lang.LobbyColumnTrnyPlayer, U.lang.LobbyColumnTrnyTable, U.lang.LobbyColumnTrnyChips, U.lang.LobbyColumnTrnyRank];
        ...
        g.sitnGoPlayerGrid = new ay($("#SitnGoPlayerGrid", g.sitnGoPlayers.$dialog), U.data.SitnGoPlayer, null, function(bh) {
        ...
        g.sitnGoWaitGrid = new ay($("#SitnGoWaitGrid", g.sitnGoPlayers.$dialog), U.data.SitnGoWait)
    };
    ...
    bd.prototype.soundVolumeChange = function(g) {
        this.soundEffects.controls.$soundPercent.text(U.lang.SoundVolume + " " + Math.round(g * 100) + "%")
    };
    ...
    bd.prototype.startGameShow = function() {
        ...
        if (g == "") {
            bh.messageShow((bg == 1) ? U.lang.LobbyCaptionNoRingGame : U.lang.LobbyCaptionNoTournament);
            return
        }
    ...
    bd.prototype.tableSelectCreate = function() {
        ...
        U.data.TableSelect.headers = [U.lang.LobbyCaptionSelect, U.lang.LobbyCaptionPlayers];
        ...
        bh.controls.tableSelectGrid = new ay($("#TableSelectGrid"), U.data.TableSelect, null, function() {
            bg.tableSelectOk()
        });
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
            bg.tableSelectOk()
    ...
    bd.prototype.tableSettingsCreate = function() {
        ...
        bi.controls.bringToFront = new E($("#BringToFront"), U.lang.TableSettingsFront);
        bi.controls.preferredSeat = new E($("#PreferredSeat"), U.lang.TableSettingsSeat);
        bi.controls.handHelper = new E($("#HandHelper"), U.lang.TableSettingsHandHelper);
        bi.controls.autoMuck = new E($("#AutoMuck"), U.lang.TableSettingsAutoMuck);
        bi.controls.dealFaceDown = new E($("#DealFaceDown"), U.lang.TableSettingsFaceDown);
        bi.controls.muteDealer = new E($("#MuteDealer"), U.lang.TableSettingsMuteDealer);
        bi.controls.rules6Holdem = new E($("#Rules6Holdem"), U.lang.TableSettings6Holdem);
        bi.controls.rules6Holdem.show(U.params.sixPlus);
        bi.controls.deckBtn = new A($("#DeckBtn"), U.lang.TableSettingsCardDeck, 20, function() {
        ...
        for (bg = 1; bg < 5; bg++) {
            bi.controls.preFlopBtn[bg] = new A($("#PreFlopBtn" + bg), "", 20, bh.preFlopButtonsCaller(bh, bg))
        }
        bi.controls.preFlopReset = new A($("#PreFlopReset"), U.lang.TableSettingsResetBtns, 20, function() {
        ...
        for (bg = 1; bg < 5; bg++) {
            bi.controls.postFlopBtn[bg] = new A($("#PostFlopBtn" + bg), "", 20, bh.postFlopButtonsCaller(bh, bg))
        }
        bi.controls.postFlopReset = new A($("#PostFlopReset"), U.lang.TableSettingsResetBtns, 20, function() {
    ...
    bd.prototype.tableSettingsShow = function() {
        ...
        bi.controls.deckBtn.setCaption(U.lang.TableSettingsCardDeck + ": #" + U.pset.Deck);
        for (bg = 1; bg < 5; bg++) {
    ...
    bd.prototype.tourneyFilterCreate = function() {
        ...
        bh.controls.tfOmahaXHiLo = new E($("#tf_omahaxhilo"), U.lang.FilterOmahaXHiLo);
        ...
        bh.controls.tfHidePrivate = new E($("#tf_hideprivate"), U.lang.FilterPrivate);
        bh.controls.tfHideRunning = new E($("#tf_hiderunning"), U.lang.FilterRunning);
    ...
    bd.prototype.tourneyFilterData = function() {
        ...
        if (!U.pset.filterTourneyActivate) {
            ...
            for (bh = 0; bh < U.data.Tourney.urows.length; bh++) {
                ...
                if (!U.pset.filterTourneyFreezeout && !bn.rebuy && !bn.reentry && !bn.shootout) {
                    continue
                }
    ...
    bd.prototype.tourneyPlayersCreate = function() {
        ...
        bg.tourneyPrevBtn = new A($("#TourneyPrevBtn", g), U.lang.LobbyButtonTrnyPrev, 25, function() {
            bg.tourneyPrev()
        });
        bg.tourneyNextBtn = new A($("#TourneyNextBtn", g), U.lang.LobbyButtonTrnyNext, 25, function() {
            bg.tourneyNext()
        });
        bg.tourneyRegister2Btn = new A($("#TourneyRegister2Btn", g), U.lang.LobbyButtonTrnyRegister, 25, function() {
            bg.tourneyRegister()
        });
        bg.tourneyStartNow = new E($("#TourneyStartNow", g), U.lang.LobbyCaptionStartNow + " *", function() {
    ...
    bd.prototype.tourneyRegOptionShow = function(bl, bk, bi, bh, g) {
        ...
        bj.data.tourney = bl;
        bj.data.needpw = bk;
        bj.controls.useTicket.setCaption(U.lang.BuyInUseTicket.split("%1%").join(bi));
        bj.controls.useBalance.setCaption(U.lang.BuyInPay.split("%1%").join(bh).split("%2%").join(g));
    ...
    bd.prototype.tourneyTabSetup = function() {
        ...
        U.data.Tourney.headers = [U.lang.LobbyColumnTrnyID, U.lang.LobbyColumnTrnyGame, U.lang.LobbyColumnTrnyBuyIn, U.lang.LobbyColumnTrnyTS, U.lang.LobbyColumnTrnyReg, U.lang.LobbyColumnTrnyStarts];
        U.data.Tourney.fields = ["id", "game", "gameIndex", "buyin", "buyinSort", "ts", "reg", "regSort", "tables", "starts", "startSort", "startMin", "startTime", "password"];
        U.data.Tourney.fieldsShow = ["id", "game", "buyin", "ts", "reg", "starts"];
        U.data.Tourney.fieldsSort = ["id", "game", "buyinSort", "ts", "regSort", "startsSort"];
        ...
        g.tourneyGrid = new ay($("#TourneyGrid", g.$dialog), U.data.Tourney, function(bh) {
            g.tourneySelect(bh)
        }, function(bh) {
        ...
        g.tourneyFilterBtn = new A($("#TourneyFilterBtn", g.$dialog), bg, 40, function() {
            g.tourneyFilterShow(false)
        });
        g.tourneyObserveBtn = new A($("#TourneyObserveBtn", g.$dialog), U.lang.LobbyButtonTrnyObserve, 40, function() {
            g.tourneyTableSelectedOpen()
        });
        g.tourneyInfoBtn = new A($("#TourneyInfoBtn", g.$dialog), U.lang.LobbyButtonTrnyInfo, 40, function() {
            g.tourneyInfoRequest()
        });
        g.tourneyPlayersBtn = new A($("#TourneyPlayersBtn", g.$dialog), U.lang.LobbyButtonTrnyPlayers, 40, function() {
            g.tourneyPlayersShow()
        });
        g.tourneyRegisterBtn = new A($("#TourneyRegisterBtn", g.$dialog), U.lang.LobbyButtonTrnyRegister, 40, function() {
            g.tourneyRegister()
        });
        g.tourneySelected = "";
        ...
        U.data.TourneyPlayer.headers = [U.lang.LobbyColumnTrnyPlayer, U.lang.LobbyColumnTrnyTable, U.lang.LobbyColumnTrnyChips, U.lang.LobbyColumnTrnyRank];
        ...
        g.tourneyPlayerGrid = new ay($("#TourneyPlayerGrid", g.tourneyPlayers.$dialog), U.data.TourneyPlayer, null, function(bh) {
        ...
        g.tourneyWaitGrid = new ay($("#TourneyWaitGrid", g.tourneyPlayers.$dialog), U.data.TourneyWait)
    };
    ...
    bd.prototype.transactionsCreate = function() {
        ...
        U.data.Trans.headers = [U.lang.TransactionsDate, U.lang.TransactionsCurrency, U.lang.TransactionsChange, U.lang.TransactionsBalance, U.lang.TransactionsSource];
        U.data.Trans.fields = ["date", "currency", "change", "balance", "source"];
        U.data.Trans.fieldsShow = ["date", "currency", "change", "balance", "source"];
        U.data.Trans.fieldsSort = ["date", "currency", "change", "balance", "source"];
        ...
        bh.controls.$count = $("#trans_count", g).text(U.lang.TransactionsRecords + " 0");
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
            bh.close()
        });
        new A($(".save", g), U.lang.DialogSave, 25, function() {
    ...
    bd.prototype.updateLobbyTitle = function() {
        ...
        if (U.mobile) {
            ...
            if (bg.lobbyTabs.getTab() == 4 && U.tables.length > 0) {
                ...
                if (U.loggedIn) {
                    g = g + " - " + U.lang.TableCaptionLoggedIn.split("%1%").join(U.loginData.player)
                }
            }
            bg.$sitePanel.text("");
            ...
            if (U.loggedIn) {
                g = U.lang.LobbyCaptionTitleLogged.split("%1%").join(U.loginData.player)
            } else {
    ...
    function an(bC, br, bh, bA, bx, bl, bj, bn, bo, g, bp, bw, bs, bu, by, bg, bt, bi, bq) {
        var bv, bz, bk, bB, bm;
        bv = this;
        bv.closed = false;
        bv.id = br;
        ...
        bv.$dialog = $(".table").clone().removeClass("table").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient).css({
            left: bi,
            top: bq
        }).attr("id", bh + br);
        ...
        bv.$chatEditFrame = $(".chateditframe", bv.$content).css("border-color", U.color.Window);
        bv.chatEdit = new ax($(".tablechatedit", bv.$chatEditFrame), {
            onEnterKey: function() {
                bv.chatSend()
            }
        });
        bv.$statsHeader = $(".tablestatsheader", bv.$chatEditFrame).text(U.lang.TableCaptionStatistics).css({
            color: U.color.ListText,
            "background-color": U.color.List
        });
        new A($(".tablechatsendbtn", bv.$chatEditFrame), "&#8595;", 20, function() {
            bv.chatSend()
        }).$button.css("border-radius", "0px");
        bv.$chatTextFrame = $(".chattextframe", bv.$content).css("border-color", U.color.Window);
    ...
    an.prototype.addonShow = function(bj, bi) {
        var bg, g, bh;
        bg = this;
        g = $(".addon").clone().removeClass("addon").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        if (!bi || bi == "") {
            bi = U.lang.DialogMessage
        }
        bh = new Y(g, bg, {
    ...
    an.prototype.addRingChipsCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".addringchips").clone().removeClass("addringchips").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: bg.id
        });
        $(".arc_addchips", g).text(U.lang.ChipsTitle);
        bh.controls.chipsMin = new C($(".arc_buyinmin", g), U.lang.ChipsMin, function() {
            bh.controls.chipsInput.setText("")
        });
        bh.controls.chipsMax = new C($(".arc_buyinmax", g), U.lang.ChipsMax, function() {
            bh.controls.chipsInput.setText("")
        });
    ...
    an.prototype.addTourneyChipsCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".yesno").clone().removeClass("yesno").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: bg.id
        });
        new A($(".ok", g), U.lang.DialogOK, 25, function() {
    ...
    an.prototype.betButtonsVisible = function() {
        return (this.button1.isVisible() || this.button2.isVisible() || this.button3.isVisible() || this.button4.isVisible())
    };
    ...
    an.prototype.betShow = function(bl, bg) {
        ...
        bh.$betLabel[bl].text(B(bg)).xytrans(0).css(bh.betLabelCSS(bl)).toggle(bg > 0)
    };
    ...
    an.prototype.buyInRingChipsCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".buyinringchips").clone().removeClass("buyinringchips").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: U.lang.BuyInTitle
        });
        bh.controls.buyInMin = new C($(".brc_buyinmin", g), U.lang.BuyInMin, function() {
            bh.controls.buyInInput.setText("")
        });
        bh.controls.buyInMax = new C($(".brc_buyinmax", g), U.lang.BuyInMax, function() {
            bh.controls.buyInInput.setText("")
        });
    ...
    an.prototype.buyInRingChipsShow = function(bg, bl, bn, bj, bo, bq, bk) {
        ...
        bm.data.min = bl;
        bm.data.max = bn;
        $(".brc_instruct", bm.$dialog).text(U.lang.BuyInMessage.split("%1%").join(bg).split("%2%").join(bp.id));
        $(".brc_seconds", bm.$dialog).text(U.lang.BuyInSeconds + " " + bg);
        g = a6(B(bo), bk);
        $(".brc_balance", bm.$dialog).text(U.lang.BuyInBalance.split("%1%").join(g));
    ...
    an.prototype.chatMobileCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".mobilechat").clone().removeClass("mobilechat").appendTo(U.lobby.$openTableBox);
        bh = new Y(g, bg, {
    ...
    an.prototype.chatStatsIconSetup = function() {
        ...
        bh.$chatStatsIconHint = $("<div>").appendTo(bh.$content).addClass("tooltip").hide();
        bh.statsMode = true;
        bh.toggleStats();
        bh.$chatStatsIcon.hover(function(bj) {
    ...
    an.prototype.chatUpdate = function() {
        ...
        for (bg = 0; bg < bl.chatQueue.length; bg++) {
            ...
            if (bn.player != "") {
                bm = bm + "<font color='" + bn.color1 + "'>" + bi + bn.title + bn.player + ":  </font><font color='" + bn.color2 + "'>" + bn.text + "</font>"
            }
            bm = bm + "</span><br>"
        }
        bh.setText(bm);
    ...
    an.prototype.collectBets = function(bg) {
        ...
        for (bj = 1; bj <= bh; bj++) {
            ...
            if (bq > 0) {
                ...
                br.$bet[bj].xytrans(0).css({
                    left: bo - br.chxOfs,
                    top: bm - br.chyOfs
                }).show().redraw();
                br.$betLabel[bj].text(B(bq)).xytrans(0).css(br.betLabelCSS(bj)).show().redraw();
    ...
    an.prototype.confirmLeave = function(bi) {
        ...
        if (bh.type == "R") {
            ...
            bh.confirmLeaveDlg.showMessage(U.lang.MessageConfirmLeave, true, U.mobile)
        }
    };
    ...
    an.prototype.confirmResign = function() {
        ...
        if (g.getPlayerSeat() > 0 && g.canResign) {
            g.confirmResignDlg.showMessage(U.lang.MessageConfirmResign, true, U.mobile)
        }
    };
    an.prototype.controlsCreate = function() {
        ...
        g.foldAnyBet = new E($(".foldanybet", g.$dialog), U.lang.TableCaptionFoldAnyBet, function(bg) {
            g.foldAnyBetClick(bg)
        }).show(false);
        g.outNextHand = new E($(".outnexthand", g.$dialog), U.lang.TableCaptionAwayHand, function(bg) {
            g.outNextHandClick(bg)
        }).show(false);
        g.outNextBlind = new E($(".outnextblind", g.$dialog), U.lang.TableCaptionAwayBlind, function(bg) {
            g.outNextBlindClick(bg)
        }).show(false);
        g.straddle = new E($(".straddle", g.$dialog), U.lang.TableCaptionStraddle, function(bg) {
            g.straddleClick(bg)
        }).show(false);
        g.icmChop = new E($(".icmchop", g.$dialog), "", function(bg) {
            g.icmChopClick(bg)
        }).show(false);
        g.runTwice = new E($(".runtwice", g.$dialog), U.lang.TableCaptionRunTwice, function(bg) {
            g.runTwiceClick(bg)
        }).show(false);
        ...
        g.$raiseBox = $(".raisebox", g.$dialog).hide().css({
            "background-color": U.color.List,
        ...
        g.timeBankBtn = new A($(".timebankbtn", g.$dialog), U.lang.TableButtonTime, 25, function() {
            g.timeBankBtn.show(false);
            g.sendButton("Time", 0)
        });
        g.timeBankBtn.show(false);
    ...
    an.prototype.dealCards = function(bi) {
        ...
        for (bs = 1; bs <= bp; bs++) {
            ...
            for (bq = 1; bq <= bt.holeCards; bq++) {
                bt.card[bq][bs].setCard(53).shade(false).moveTo(0, bl, bj).clip(false).show(false)
            }
        }
        bg = 250;
    ...
    an.prototype.dealFlop = function(g, bj) {
        ...
        for (bh = 1; bh <= 3; bh++) {
            ...
            if (g == 2) {
                ...
                bi.board2[bh].moveTo(0, 350 - bi.cxOfs, 5).show(true).redraw();
                bi.board2[bh].moveTo(bl, bi.boardX[bh] - bi.cxOfs, bi.board2Y - bi.cyOfs)
            } else {
                bi.board[bh].setCard(53).shade(false);
                bi.board[bh].moveTo(0, 350 - bi.cxOfs, 5).show(true).redraw();
                bi.board[bh].moveTo(bl, bi.boardX[bh] - bi.cxOfs, bi.boardY - bi.cyOfs - bk)
            }
        }
        ...
        function bg() {
            ...
            for (bm = 1; bm <= 3; bm++) {
                ...
                if (g == 2) {
                    bi.board2[bm].moveTo(0, bi.boardX[bm] - bi.cxOfs, bi.board2Y - bi.cyOfs).redraw();
                    bi.board2[bm].setCard(bi.boardCard2[bm])
                } else {
                    bi.board[bm].moveTo(0, bi.boardX[bm] - bi.cxOfs, bi.boardY - bi.cyOfs - bk).redraw();
    ...
    an.prototype.dealPreFlop = function() {
        ...
        bg.board[1].moveTo(0, 350 - bg.cxOfs, 5).show(true).redraw();
        bg.board[1].moveTo(bh, bg.boardX[1] - bg.cxOfs, bg.boardY - bg.cyOfs);
        setTimeout(g, bh + 100);

        function g() {
            bg.board[1].moveTo(0, bg.boardX[1] - bg.cxOfs, bg.boardY - bg.cyOfs).redraw();
    ...
    an.prototype.dealRiver = function(g) {
        ...
        if (g == 2) {
            ...
            bh.board2[5].moveTo(0, 350 - bh.cxOfs, 5).show(true).redraw();
            bh.board2[5].moveTo(bk, bh.boardX[bj] - bh.cxOfs, bh.board2Y - bh.cyOfs)
        } else {
            bh.board[5].setCard(53).shade(false);
            bh.board[5].moveTo(0, 350 - bh.cxOfs, 5).show(true).redraw();
            bh.board[5].moveTo(bk, bh.boardX[bj] - bh.cxOfs, bh.boardY - bh.cyOfs - bi)
        }
        setTimeout(bg, bk + 100);

        function bg() {
            if (g == 2) {
                bh.board2[5].moveTo(0, bh.boardX[bj] - bh.cxOfs, bh.board2Y - bh.cyOfs).redraw();
                bh.board2[5].setCard(bh.boardCard2[5])
            } else {
                bh.board[5].moveTo(0, bh.boardX[bj] - bh.cxOfs, bh.boardY - bh.cyOfs - bi).redraw();
    ...
    an.prototype.dealStreet = function(bh, g) {
        ...
        for (bk = 1; bk <= bi; bk++) {
            if (g[bk] != 0) {
                bo.card[bh][bk].setCard(53).shade(false).moveTo(0, bn, bl).clip(false).show(false)
            }
        }
        bj = 250;
    ...
    an.prototype.dealTurn = function(g) {
        ...
        if (g == 2) {
            ...
            bh.board2[4].moveTo(0, 350 - bh.cxOfs, 5).show(true).redraw();
            bh.board2[4].moveTo(bj, bh.boardX[4] - bh.cxOfs, bh.board2Y - bh.cyOfs)
        } else {
            bh.board[4].setCard(53).shade(false);
            bh.board[4].moveTo(0, 350 - bh.cxOfs, 5).show(true).redraw();
            bh.board[4].moveTo(bj, bh.boardX[4] - bh.cxOfs, bh.boardY - bh.cyOfs - bi)
        }
        setTimeout(bg, bj + 100);

        function bg() {
            if (g == 2) {
                bh.board2[4].moveTo(0, bh.boardX[4] - bh.cxOfs, bh.board2Y - bh.cyOfs).redraw();
                bh.board2[4].setCard(bh.boardCard2[4])
            } else {
                bh.board[4].moveTo(0, bh.boardX[4] - bh.cxOfs, bh.boardY - bh.cyOfs - bi).redraw();
    ...
    an.prototype.drawTable = function() {
        ...
        for (bm = 1; bm <= 5; bm++) {
            ...
            if (bl != 0) {
                ...
                bt.board[bm].moveTo(0, bt.boardX[bm] - bt.cxOfs, bt.boardY - bt.cyOfs - g).show(true)
            }
            if (bj != 0 && bl != bj) {
                bt.board2[bm].moveTo(0, bt.boardX[bm] - bt.cxOfs, bt.board2Y - bt.cyOfs).show(true)
            }
        }
        for (bm = 1; bm < bh; bm++) {
        ...
        for (bm = 1; bm <= bh; bm++) {
            ...
            if (bi != "") {
                ...
                if (bs == 0) {
                    bt.seat[bm].avatarSetCustom(bt.playerName[bm], bt.playerAvatarCrc[bm])
                } else {
                ...
                if (bk >= 0) {
                    bt.seat[bm].setNoteColor(U.data.Notes.rows[bk].noteText, U.data.Notes.rows[bk].colorNum)
                }
                for (bk = 1; bk <= bt.holeCards; bk++) {
    ...
    an.prototype.getPasswordCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".tablepassword").clone().removeClass("tablepassword").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: U.lang.PasswordTitle
        });
    ...
    an.prototype.guiChange = function() {
        ...
        g.resizeFinish();
        g.bringToFront();
        g.addRingChips.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.addTourneyChips.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.buyInRingChips.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.confirmFold.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.confirmLeaveDlg.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.confirmResignDlg.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.leaveDlg.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.getPassword.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.infoDialog.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.playerInfo.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.rotateDialog.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.colorLabel.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.playerNote.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.tourneyChop.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        g.replayDialog.$dialog.appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient)
    };
    ...
    an.prototype.historyLabel = function(bi) {
        ...
        bg = Math.round(bi * (bk - 1));
        g = bg + 1;
        bj.controls.$historyNumber.text(bh.hhNumbers[bg] + "  (" + U.lang.InfoHistoryOf.split("%1%").join(g).split("%2%").join(bk) + ")")
    };
    ...
    an.prototype.iconMenuSetup = function() {
        ...
        bi.$menuIconHint = $("<div>").appendTo(bi.$content).addClass("tooltip").hide();
        if (U.params.gradients) {
            $(".icontray", bi.$dialog).css("background-image", "url('Image?Name=Grad30')")
        }
        if (a1(U.color.Button) > 127) {
            bk = 0
        } else {
    ...
    an.prototype.infoInit = function() {
        var bi, bg, bj, g, bh;
        bi = this;
        bg = $(".tableinfo").clone().removeClass("tableinfo").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bj = new Y(bg, bi, {
        ...
        bj.controls.historySlider = new ao($(".historyslider", bg), 1, function(bk) {
            bi.historyChange(bk)
        });
        ...
        bj.controls.chatInfoMove = new E($(".chatinfomove", bg), U.lang.InfoMoveChat, function() {
            bi.moveChat()
        });
    ...
    an.prototype.infoUpdate = function() {
        ...
        if (U.mobile && U.tableCurrent == U.tables.indexOf(bg)) {
            ...
            g = g.split("<pre>").join("").split("</pre>").join("").split("\n").join("<br>");
    ...
    an.prototype.leaveCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".tableleave").clone().removeClass("tableleave").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: bg.id
        });
        bh.controls.seatbtn = new A($(".tl_seat", g), U.lang.TableMenuLeaveSeat, 25, function() {
            bh.close();
            bg.confirmLeave(true)
        });
        bh.controls.tablebtn = new A($(".tl_table", g), U.lang.TableMenuLeaveTable, 25, function() {
            bh.close();
            bg.confirmLeave(false)
        });
        bh.controls.trnybtn = new A($(".tl_trny", g), U.lang.TableMenuLeaveTourney, 25, function() {
            bh.close();
            bg.confirmResign()
        });
        new A($(".cancelbtn", g), U.lang.DialogCancel, 25, function() {
    ...
    an.prototype.leaveShow = function() {
        ...
        bg.controls.trnybtn.show(g.getPlayerSeat() > 0 && g.type == "T" && g.canResign);
        bg.show(true, U.mobile)
    };
    an.prototype.makeGraphics = function() {
        ...
        for (bj = 1; bj <= bh; bj++) {
            ...
            for (bi = 1; bi <= 7; bi++) {
                ...
                bo.card[bi][bj] = new F(bo, bk, bn - bo.holeX[bi], bl - bo.holeY[bi], bj, bi);
            ...
            bn = bo.chipX[bh][g];
            bl = bo.chipY[bh][g];
            bo.$bet[bj] = $("<div>").addClass("chipstack").css({
                left: bn - bo.chxOfs,
                top: bl - bo.chyOfs
            }).appendTo(bo.$content);
            bo.$betLabel[bj] = $("<div>").css({
                color: bo.oncolor
            }).appendTo(bo.$content)
        }
        for (bj = 1; bj < bh; bj++) {
            ...
            bo.$pot[bj] = $("<div>").addClass("chipstack").css({
                left: bn - bo.chxOfs,
                top: bl
            }).appendTo(bo.$content);
            bl = bo.potY[bj] + bo.chyOfs;
            bo.$potLabel[bj] = $("<div>").css({
                left: bn,
                top: bl,
                color: bo.oncolor
            }).appendTo(bo.$content)
        ...
        bo.$dealer = $("<div>").addClass("dealer").css({
            left: bn,
            top: bl,
            background: "url('Image?Name=Chips&Crc=" + U.crc.image + "') 0px 0px/auto 19px no-repeat"
        }).appendTo(bo.$content);
    ...
    an.prototype.messageShow = function(bj, bi) {
        var bg, g, bh;
        bg = this;
        g = $(".message").clone().removeClass("message").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        if (!bi || bi == "") {
            bi = U.lang.DialogMessage
        }
        bh = new Y(g, bg, {
    ...
    an.prototype.mixedChoiceCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".mixedchoice").clone().removeClass("mixedchoice").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: bg.id
        });
        bh.data = {};
    ...
    an.prototype.playerInfoCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".playerinfo").clone().removeClass("playerinfo").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {});
        bh.data.piName = "";
        bh.controls.piProfile = new A($(".pi_profile", g), U.lang.PlayerProfile, 25, function() {
            a0(bh.data.piName)
        });
        bh.controls.piNotes = new A($(".pi_notes", g), U.lang.PlayerNotes, 25, function() {
            bh.close();
            ac(bg, bh.data.piName)
        });
        bh.controls.piSearch = new A($(".pi_search", g), U.lang.PlayerSearch, 25, function() {
            bg.playerInfoSearch()
        });
        bh.data.piHereValue = 0;
        bh.controls.piRotate = new A($(".pi_rotate", g), U.lang.RotateHere, 25, function() {
    ...
    an.prototype.potAward = function(bg) {
        ...
        for (bk = 1; bk <= bh; bk++) {
            ...
            if (bt > 0) {
                ...
                bu.$bet[bk].xytrans(0).css({
                    left: bu.potX[bm] - bu.chxOfs,
                    top: bu.potY[bm] - bu.chyOfs
                }).show().redraw();
                bu.$betLabel[bk].xytrans(0).css({
                    left: bu.potX[bm] - bu.chxOfs,
                    top: bu.potY[bm] + bu.chyOfs
                }).text(B(bt)).show().redraw();
                br = bu.chipX[bh][g];
                bo = bu.chipY[bh][g];
    ...
    an.prototype.replayCreate = function() {
        ...
        bo = this;
        bg = $(".tablereplay").clone().removeClass("tablereplay").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        ...
        bk.controls.$rabbit = [];
        for (bi = 0; bi < 5; bi++) {
            bk.controls.$ccard[bi] = $("<div>").appendTo(bm).css({
                left: 20 + bi * 49,
                top: 15,
                width: 46,
                height: 64
            }).hide();
            bk.controls.$rabbit[bi] = $("<div>").addClass("replayrabbit").appendTo(bk.controls.$ccard[bi]).hide();
            bk.controls.$ccard2[bi] = $("<div>").appendTo(bm).css({
                left: 20 + bi * 49,
                top: 47,
                width: 46,
                height: 36
        ...
        for (bj = 0; bj < 10; bj++) {
            ...
            for (bi = 0; bi < 7; bi++) {
                bk.controls.$card[bj][bi] = $("<div>").appendTo(g[bj]).css({
                    left: 238 + bi * 14,
                    top: 2,
                    width: 46,
                    height: 36
                }).hide();
                bk.controls.$cardshade[bj][bi] = $("<div>").addClass("replayshade").appendTo(bk.controls.$card[bj][bi])
            }
        ...
        bk.controls.rabbit = new E($(".tr_rabbit", bg), U.lang.ReplayRabbit, function() {
    ...
    an.prototype.replayUpdate = function(bn) {
        ...
        for (bq = 0; bq < 5; bq++) {
            ...
            if (bq < bi) {
                ...
                bu.controls.$ccard[bq].css("background-position", ((g - 1) * -46) + "px 0px").toggle(!bv.Rabbit[bq] || bp);
                ...
                bu.controls.$ccard2[bq].css("background-position", ((bl - 1) * -46) + "px 0px").show()
            }
        }
        for (bq = 0; bq < 10; bq++) {
            if (bq < bs.seats) {
                ...
                if (bq + 1 == bv.Dealer) {
                    bu.controls.$dealer[bq].css("background", "url('Image?Name=Chips&Crc=" + U.crc.image + "') 0px 0px/auto 19px no-repeat")
                } else {
                ...
                bj = bv.Avatar[bq];
                if (bj == 0) {
                    bu.controls.$avatar[bq].css("background", "url('Avatar?Player=" + encodeURIComponent(bv.Name[bq]) + "&Crc=" + bv.AvatarCrc[bq] + "') 0px 0px/36px 36px no-repeat")
                } else {
                    bu.controls.$avatar[bq].css("background", "url('Image?Name=Avatars&Crc=" + U.crc.image + "') " + ((bj - 1) * -36) + "px 0px/auto 36px no-repeat")
                }
                bu.controls.$name[bq].html(bv.Name[bq] == "" ? "" : bv.Name[bq] + "<br>" + bv.Stack[bq]);
                ...
                for (bo = 0; bo < 7; bo++) {
                    if (bo < bg) {
                        ...
                        bu.controls.$card[bq][bo].css("background-position", ((g - 1) * -46) + "px 0px").show();
        ...
        bo = bq + 1;
        bu.controls.$hand.text(U.lang.TableMenuHeaderHand + " #" + bv.Hand + " (" + U.lang.InfoHistoryOf.split("%1%").join(bo).split("%2%").join(bm) + ")");
        bt = U.lang.ReplayPots + " " + bv.Pots + " &nbsp; " + U.lang.ReplayTotal + " " + bv.Total;
        if (bs.type == "R") {
    ...
    an.prototype.resizeTable = function(bm) {
        ...
        if (U.mobile) {
            ...
            if (bm == "n-resize" || bm == "s-resize" || bm == "nw-resize" || bm == "ne-resize") {
    ...
    an.prototype.rotateSeats = function(bh, bl) {
        ...
        bo.controls.$rsStatus.text(U.lang.RotateStatus.split("%1%").join(bp.rotate));
    ...
    an.prototype.rotateSeatsCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".rotateseats").clone().removeClass("rotateseats").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
    ...
    an.prototype.rules6HoldemCreate = function() {
        var bg, g, bi, bh;
        bg = this;
        g = $(".tablerules6holdem").clone().removeClass("tablerules6holdem").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bi = new Y(g, bg, {
            title: bg.id
        });
    ...
    an.prototype.setHint = function(bl) {
        ...
        if (bi == "") {
            ...
            bm = bm + "<br>" + U.lang.MouseOverChips + ": " + B(bv.playerChips[bl]) + ",  " + U.lang.MouseOverTimeBank + ": " + bv.playerTime[bl];
            if (bt != "") {
            ...
            bu = bu + bp;
            if (bu != "") {
                bm = bm + "<br>" + U.lang.NoteNote + ": " + bc(bu).split("\n").join("<br>")
            }
        }
        bv.seat[bl].hint = bm
    };
    ...
    an.prototype.showChat = function(g) {
        ...
        if (g) {
            ...
            bg.timeBankBtn.move(365, 440).resize(125, 27, 25).lineHeight("25px").fontSize("1em");
            bg.button1.move(365, 472).resize(100, 32, 30).lineHeight("14px").fontSize("1em");
            bg.button2.move(475, 472).resize(100, 32, 30).lineHeight("14px").fontSize("1em");
            bg.button3.move(585, 472).resize(100, 32, 30).lineHeight("14px").fontSize("1em");
            bg.button4.move(585, 435).resize(100, 32, 30).lineHeight("14px").fontSize("1em")
        } else {
            bg.straddle.$container.css({
                top: 335,
                fontSize: "1em"
            ...
            bg.raiseSlider.updateThumb();
            bg.timeBankBtn.move(15, 462).resize(160, 42, 40).lineHeight("20px").fontSize("1.2em");
            bg.button1.move(185, 462).resize(160, 42, 40).lineHeight("20px").fontSize("1.2em");
            bg.button2.move(355, 462).resize(160, 42, 40).lineHeight("20px").fontSize("1.2em");
            bg.button3.move(525, 462).resize(160, 42, 40).lineHeight("20px").fontSize("1.2em");
            bg.button4.move(525, 418).resize(160, 37, 35).lineHeight("20px").fontSize("1.2em")
        }
    };
    ...
    an.prototype.toggleSelect = function(bh, bg) {
        var g;
        g = this;
        if (bh == 0 || bg == 0 || g.getPlayerSeat() != bh || g.cardNum[bg] == 0 || g.ghosted || g.isFaceDown) {
            return
        }
    ...
    an.prototype.toggleStats = function() {
        ...
        g.$chatStatsIcon.css("background", "url('Image?Name=ChatStats') " + bg + "px 0px/auto 20px no-repeat");
    ...
    an.prototype.totalPlateSetup = function() {
        ...
        bh.$totalPlate.hover(function(bj) {
    ...
    an.prototype.tourneyChopCreate = function() {
        var bg, g, bh;
        bg = this;
        g = $(".tourneychop").clone().removeClass("tourneychop").appendTo(U.mobile ? U.lobby.$openTableBox : U.$webClient);
        bh = new Y(g, bg, {
            title: bg.id
        });
        ...
        bh.data.Chop.headers = [U.lang.ChopPlace, U.lang.ChopPayout, U.lang.ChopPlayer, U.lang.ChopStack, U.lang.ChopChop];
    ...
    an.prototype.updateTotal = function() {
        ...
        if (g.totalPot == "") {
            ...
            g.$totalPlate.html(U.lang.TableCaptionTotal + "<br>" + B(g.totalPot)).show();
            if (g.totalPot == 0) {
                g.totalRake = 0
            }
            g.$totalTip.html(U.lang.TableCaptionRake.split("%1%").join(B(g.totalRake)))
        }
    };
    ...
    function Y(bi, bh, g) {
        ...
        $(".header > .title", bg.$dialog).css("color", U.color.WindowText).text(bg.title);
    ...
    Y.prototype.onDialog = function(bi) {
        ...
        if (bj.resizing) {
            ...
            bn = g.pageX - bj.xdown;
            bm = g.pageY - bj.ydown;
            if (bj.rmode == "e-resize" || bj.rmode == "ne-resize" || bj.rmode == "se-resize") {
                bl = bj.wdialog + bn;
                if (bl < bj.minwidth) {
                    bl = bj.minwidth
                }
            ...
            if (bj.rmode == "s-resize" || bj.rmode == "sw-resize" || bj.rmode == "se-resize") {
                bh = bj.hdialog + bm;
                if (bh < bj.minheight) {
                    bh = bj.minheight
                }
            ...
            if (bj.rmode == "w-resize" || bj.rmode == "nw-resize" || bj.rmode == "sw-resize") {
                bl = bj.wdialog - bn;
                if (bl >= bj.minwidth) {
                    bj.$dialog.css({
                        left: g.pageX,
            ...
            if (bj.rmode == "n-resize" || bj.rmode == "nw-resize" || bj.rmode == "ne-resize") {
                bh = bj.hdialog - bm;
                if (bh >= bj.minheight) {
                    bj.$dialog.css({
                        top: g.pageY,
    ...
    Y.prototype.show = function(bo, bp) {
        ...
        if (bp) {
            ...
            if (bm.parent == null) {
                ...
                bh = bm.parent.$dialog.position().left + (bm.parent.$dialog.width() - bg) / 2;
                if (bh < 0) {
                    bh = 0
                }
                bn = bm.parent.$dialog.position().top + (bm.parent.$dialog.height() - br) / 2;
                if (bn < 0) {
                    bn = 0
                }
                if (bm.scaleX == 1) {
    ...
    Y.prototype.showMessage = function(bg, bo, bp) {
        ...
        if (bp) {
            ...
            bh = bl.parent.$dialog.position().left + (bl.parent.$dialog.width() - g) / 2;
            if (bh < 0) {
                bh = 0
            }
            bm = bl.parent.$dialog.position().top + (bl.parent.$dialog.height() - bq) / 2;
            if (bm < 0) {
                bm = 0
            }
            bl.$dialog.css({
    ...
    function ak(bg, bh) {
        var g, bi;
        g = $(".colorlabel").clone().removeClass("colorlabel").appendTo(U.mobile && bh ? U.lobby.$openTableBox : U.$webClient);
        bi = new Y(g, bg, {
    ...
    function u(bg, bi) {
        var g, bj, bh;
        g = $(".playernote").clone().removeClass("playernote").appendTo(U.mobile && bi ? U.lobby.$openTableBox : U.$webClient);
        bj = new Y(g, bg, {});
        ...
        bj.controls.colorGrid = new ay($(".pn_colorgrid", g), bj.data.Colors, null, function() {
            aZ(bg)
        }, null);
    ...
    function a(bg) {
        ...
        if (bn == bj.data.oldChat && bh == bj.data.oldColor && bk == bj.data.oldNote) {
            bj.close();
            return
        }
        bl = {};
